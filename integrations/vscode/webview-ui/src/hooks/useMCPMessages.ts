...
export function useMCPMessages(vscodeApi: VSCodeAPI): void {
    ...
    const setPinnedTranscript = useStore((s) => s.setPinnedTranscript);
    const setQualityDashboard = useStore((s) => s.setQualityDashboard);
    ...
    useEffect(() => {
        const handler = (event: MessageEvent) => {
            ...
            switch (msg.type) {
                ...
                case 'qualityDashboardUpdate':
                    if (msg.dashboard) {
                        setQualityDashboard(msg.dashboard);
                    }
                    break;

                case 'pinnedTranscriptUpdate':
                    if (msg.transcript) {
                        setPinnedTranscript(msg.transcript);
                    }
                    break;
